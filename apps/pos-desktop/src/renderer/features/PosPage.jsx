...
import { loadPosBootstrap } from "@/lib/actions/posBootstrap";
...
function ProductThumb({ item, className = "h-10 w-12" }) {
  return item?.imageUrl ? (
    <img src={item.imageUrl} alt="" className={`${className} flex-shrink-0 rounded-md object-cover`} />
  ) : (
    <span className={`${className} grid flex-shrink-0 place-items-center rounded-md bg-slate-100 text-slate-400`}>
...
const MenuItem = React.memo(({ item, index, onAdd, onTogglePin }) => {
  ...
  return (
    <motion.div
      className="relative group menu-item-virtual"
    >
      {/* Pin Button */}
      <motion.button
        onClick={(e) => {
      ...
      >
        ...
        <div className="mb-2 sm:mb-3 aspect-[4/3] w-full overflow-hidden rounded-md bg-slate-100">
          {item.imageUrl && !imageFailed ? (
            ...
            <div className="grid h-full place-items-center text-slate-400">
              <ImageIcon className="h-9 w-9 sm:h-11 sm:w-11" />
            </div>
          )}
        </div>
...
export default function POSPage() {
  ...
  const [kitchensCache, setKitchensCache] = useState([]);
  //  NEW: Cache kitchens for fast access
  ...
  const [printQueue, setPrintQueue] = useState({ queued: 0, printing: 0, failed: 0, recentJobs: [] });
  ...
  const [selectedEditItemForQuantity, setSelectedEditItemForQuantity] =
    useState(null);
  ...
  useEffect(() => {
    const performSearch = async () => {
      ...
      const searchTerm =
        nameQuery.length >= phoneQuery.length ? nameQuery : phoneQuery;
    ...
    const debounceTimer = setTimeout(performSearch, 80);
  ...
  // Load initial data
  useEffect(() => {
    void loadPosBootstrap().then((response) => {
      if (!response.success) {
        showNotification(response.error || "Failed to load POS", "error");
      ...
      setPinnedItems(data.menuItems.filter((item) => item.isPinned));
      setKitchensCache(data.kitchens.filter((kitchen) => kitchen.isActive));
      setPendingOrders(data.pendingOrders);
      setRestaurantSettings(data.settings);
      setOrderDetails((current) => ({ ...current, taxPercentage: data.settings.taxPercentage || 0 }));
      setIsLoadingCategories(false);
      setIsLoadingMenuItems(false);
    });
  }, []);
  ...
  const handleTogglePin = useCallback(
    async (menuItemId, currentPinStatus) => {
      try {
  ...
  const loadPendingOrders = async (silent = false) => {
    try {
      if (!silent) setIsLoadingPendingOrders(true);
      ...
      if (!silent) setIsLoadingPendingOrders(false);
    }
  };
  ...
  useEffect(() => {
    if (
      ...
      setOrderDetails((prev) => ({
        ...prev,
        deliveryCharge: restaurantSettings?.deliveryCharges || 0,
      }));
    ...
    restaurantSettings?.deliveryCharges,
  ]);
  // Calculations
  ...
  const selectedOrderType =
    ORDER_TYPES.find((type) => type.value === orderDetails.orderType) ||
    ORDER_TYPES[0];
  ...
  const handleOrderTypeChange = useCallback(
    (orderType) => {
      setOrderDetails((current) => ({
        ...
        customerName: orderType === "dine-in" ? "" : current.customerName,
        phoneNumber: orderType === "dine-in" ? "" : current.phoneNumber,
        address: orderType === "delivery" ? current.address : "",
        deliveryCharge:
          orderType === "delivery"
        ...
        taxPercentage:
          orderType === "dine-in"
            ? restaurantSettings?.taxPercentage || 0
            : 0,
      }));
  ...
  const addToCartWithQuantity = (menuItem, quantity = 1) => {
    setCart((current) => {
      ...
      return existing
        ? current.map((item) => item._id === menuItem._id
          ? { ...item, quantity: item.quantity + quantity }
          : item)
        : [...current, { ...menuItem, quantity }];
    });
  ...
  useEffect(() => {
    const refresh = async () => setPrintQueue(await window.restrozapp.print.queueStatus());
    void refresh();
    return window.restrozapp.print.onQueueChanged(() => void refresh());
  }, []);
  ...
  const submitOrder = async () => {
    ...
    if (currentDetails.orderType === "delivery") {
      if (
        !currentDetails.customerName.trim() ||
        !currentDetails.phoneNumber.trim() ||
  ...
  const finalizePendingOrder = (order) => {
    setPendingOrders((current) => [order, ...current.filter((entry) => entry._id !== order._id)]);
    setCart([]);
    setOrderDetails({
      orderType: "dine-in",
      tableNumber: "",
  ...
  const confirmCompleteOrder = async () => {
    ...
    try {
      ...
      const total =
        subtotal + tax - discountAmount + completionDetails.deliveryCharge;
...

      {
        ...
        setPendingOrders((current) => current.filter((order) => order._id !== completionConfirmation._id));
  ...
  const refreshData = async () => {
    await Promise.all([loadCategories(), loadMenuItems(), loadPendingOrders()]);
  ...
  const handlePrintSelectedKOT = async () => {
    ...
    try {
      const queued = await window.restrozapp.print.enqueue({
        orderId: editingOrder._id,
        receiptType: "kot",
        itemIds: selectedKOTItems,
      });
      if (!queued.ok) throw new Error(queued.error);
      showNotification(`KOT queued for ${selectedKOTItems.length} item(s)!`, "success");
    } catch (error) {
  ...
  const saveEditedOrder = async () => {
    ...
    try {
      ...
      const updatedItems = editCart.map((item) => ({
        ...
        imageUrl: item.imageUrl || "",
        kitchenId: item.kitchenId?._id || item.kitchenId || null,
      }));
      ...
      if (response.success) {
        ...
        setPendingOrders((current) => current.map((order) =>
  ...
  const handleReprintKOT = async (order) => {
    try {
      const queued = await window.restrozapp.print.enqueue({ orderId: order._id, receiptType: "kot" });
      if (!queued.ok) throw new Error(queued.error);
      showNotification("KOT queued for printing!", "success");
    } catch (error) {
  ...
  const handleReprintToken = async (order) => {
    try {
      const queued = await window.restrozapp.print.enqueue({ orderId: order._id, receiptType: "token" });
      if (!queued.ok) throw new Error(queued.error);
      showNotification("Token queued for printing!", "success");
    } catch (error) {
  ...
  const filteredPendingOrders = pendingOrders.filter((order) => {
    const matchesSearch =
      order.orderNumber
        .toLowerCase()
        .includes(pendingOrdersSearch.toLowerCase()) ||
      order.tempOrderNumber
        .toLowerCase()
        .includes(pendingOrdersSearch.toLowerCase()) || //  ADD THIS
      order.customerName
        .toLowerCase()
        .includes(pendingOrdersSearch.toLowerCase()) ||
    ...
    const matchesType =
      ...
      order.orderType === pendingOrderTypeFilter;

    return matchesSearch && matchesType;
  });

  useEffect(() => {
    if (!isPendingOrdersOpen) return;
    void loadPendingOrders(true);
    const timer = window.setInterval(() => void loadPendingOrders(true), 15_000);
  ...
  const confirmCancelOrder = async () => {
    ...
    try {
      ...
      if (response.success) {
        showNotification(
          `Order ${cancelConfirmation.orderNumber} cancelled`,
          "success"
        );
        setPendingOrders((current) => current.filter((order) => order._id !== cancelConfirmation.orderId));
  ...
  const handleEditSearchKeyDown = (e) => {
    ...
    if (e.key === "ArrowDown") {
      e.preventDefault();
      setSelectedEditSearchIndex((prev) =>
        prev < editSearchResults.length - 1 ? prev + 1 : prev
      );
    } else if (e.key === "ArrowUp") {
      e.preventDefault();
      ...
      if (selectedEditSearchIndex >= 0) {
        ...
        handleEditItemSelectForQuantity(
          editSearchResults[selectedEditSearchIndex]
        );
      }
    } else if (e.key === "Escape") {
      setShowEditSearchDropdown(false);
    ...
    <>
      ...
      <div className="print:hidden lg:mt-[-30px] max-h-0 bg-gradient-to-br from-slate-50 via-white to-slate-100 p-2 sm:p-3 md:p-4 lg:p-6 ">
        <div className="max-w-[2000px] mx-auto">
          ...
          {(printQueue.queued > 0 || printQueue.printing > 0 || printQueue.failed > 0) && (
            <div className={`fixed bottom-4 right-4 z-[9998] flex items-center gap-3 rounded-xl border px-4 py-3 shadow-xl ${
              printQueue.failed > 0 ? "border-red-300 bg-red-50 text-red-800" : "border-slate-200 bg-white text-slate-700"
            }`}>
              <Printer className="h-5 w-5" />
              <div className="text-xs font-semibold">
                <div>{printQueue.failed > 0 ? `${printQueue.failed} print job(s) failed` : "Printing in background"}</div>
                <div className="font-normal opacity-70">{printQueue.printing} printing · {printQueue.queued} queued</div>
              </div>
              {printQueue.failed > 0 && (
                <button
                  type="button"
                  onClick={() => void window.restrozapp.print.retryFailed()}
                ...
                <div className="mb-3 sm:mb-4">
                  <div className="flex gap-2 sm:gap-3">
                    ...
                    <div className="relative flex-1">
                      ...
                      <input
                        ...
                        onFocus={() =>
                          searchQuery && setShowSearchDropdown(true)
                        }
                            ...
                            <div className="p-1.5 sm:p-2">
                              {searchResults.map((product, index) => (
                                <motion.button
                                  key={product._id}
                                  onClick={() =>
                                    handleItemSelectForQuantity(product)
                                  }
                                  onMouseEnter={() =>
                                  ...
                                  whileHover={{ x: 4 }}
                                >
                                  <ProductThumb item={product} className="h-10 w-12" />
                      ...
                      {selectedItemForQuantity ? (
                        ...
                        >
                          ...
                          <div className="hidden sm:flex items-center gap-2 px-3 py-2 bg-gradient-to-r from-emerald-50 to-teal-50 border-2 border-emerald-500 rounded-xl">
                            <ProductThumb item={selectedItemForQuantity} className="h-9 w-11" />
                  ...
                  <AnimatePresence>
                    {selectedItemForQuantity && (
                      ...
                      >
                        <ProductThumb item={selectedItemForQuantity} className="h-10 w-12" />
                        <div className="flex-1 min-w-0">
                ...
                >
                  <div className="flex items-center gap-2 px-3 py-1.5 bg-gradient-to-r from-yellow-50 to-orange-50 border-2 border-yellow-300 rounded-lg">
                    ...
                    <span className="text-sm font-bold text-yellow-700">
                      Pinned Items (
                      {filteredProducts.filter((i) => i.isPinned).length})
                    </span>
                  </div>
                  ...
                  <AnimatePresence>
                    {cart.length === 0 ? (
                      ...
                      cart.map((item, index) => (
                        ...
                        >
                          <ProductThumb item={item} className="h-11 w-14" />
                          <div className="flex-1 min-w-0">
                    ...
                    <div className="space-y-1.5 sm:space-y-2 mb-3 sm:mb-4">
                      ...
                      <div className="flex justify-between text-xs sm:text-sm text-slate-600">
                        <span className="font-medium">
                          Service Charges ({orderDetails.taxPercentage}%):
                        </span>
                      ...
                      {orderDetails.deliveryCharge > 0 && (
                        <div className="flex justify-between text-xs sm:text-sm text-orange-600">
                          ...
                          <span className="font-bold">
                            ₨{orderDetails.deliveryCharge.toFixed(2)}
                          </span>
                        </div>
                      )}
                      {(orderDetails.discountPercentage > 0 ||
                        ...
                        <div className="flex justify-between text-xs sm:text-sm text-emerald-600">
                          <span className="font-medium">
                            Discount
                            {orderDetails.discountPercentage > 0 &&
                              ` (${orderDetails.discountPercentage.toFixed(
                                1
                              )}%)`}
                            :
                          </span>
                ...
                {[
                  {
                    keys: ["Ctrl", "F"],
                    action: "Focus Search",
                    desc: "Quick search menu items",
                  },
                  {
                    keys: ["Ctrl", "C"],
                    action: "Confirm Order",
                    desc: "Open order confirmation",
                  },
                  {
                    keys: ["Ctrl", "K"],
                    action: "Send to Kitchen",
                    desc: "Submit order (from modal)",
                  },
                  {
                    keys: ["Ctrl", "P"],
                  ...
                  },
                  {
                    ...
                    desc: "Move through search results",
                  },
                  {
                    keys: ["Enter", "Qty", "Enter"],
                    action: "Quick Add Item",
                    desc: "Search → Select → Enter quantity → Add",
                  },
                ].map((shortcut, index) => (
                  <motion.div
                    key={index}
                ...
                <div className="flex-1 min-w-0">
                  <h2 className="text-base sm:text-2xl font-bold flex items-center gap-2">
                    ...
                    <span className="truncate">
                      Confirm {selectedOrderType.label} Order
                    </span>
                  </h2>
                  <p className="text-emerald-100 text-xs sm:text-sm mt-0.5 sm:mt-1 hidden sm:block">
                    Review {selectedOrderType.label.toLowerCase()} details and submit
                  </p>
                </div>
                <button
                  onClick={() =>
                    !isSubmittingOrder && setIsOrderModalOpen(false)
                  }
                    ...
                    {cart.map((item) => (
                      ...
                      >
                        <div className="flex items-center gap-1.5 sm:gap-2 flex-1 min-w-0">
                          <ProductThumb item={item} className="h-9 w-11" />
                    ...
                    {(orderDetails.discountPercentage > 0 ||
                      ...
                      <div className="flex justify-between text-emerald-600">
                        <span>
                          Discount
                          {orderDetails.discountPercentage > 0 &&
                            ` (${orderDetails.discountPercentage.toFixed(1)}%)`}
                          :
                        </span>
                        <span className="font-semibold">
                          -₨{discountAmount.toFixed(2)}
  ...
  {orderDetails.orderType === "dine-in" && (
    <div>
      <label className="block text-slate-700 font-semibold mb-2 text-xs sm:text-sm flex items-center gap-1.5">
        ...
        {restaurantSettings?.halls?.length ? "Select Hall & Table Number" : "Table Number"}
      </label>
      
      {restaurantSettings?.halls?.length > 0 && (
        <div className="flex flex-wrap gap-2 sm:gap-3 mb-3">
          {restaurantSettings.halls.map((hall, index) => {
            const selectedHall = restaurantSettings.halls.find(
              (name) => orderDetails.tableNumber === name || orderDetails.tableNumber.startsWith(`${name} - `)
            ) || restaurantSettings.halls[0];
            return (
              <button
                key={hall}
                type="button"
                onClick={() => {
                  const table = orderDetails.tableNumber.includes(" - ")
                    ? orderDetails.tableNumber.split(" - ").slice(1).join(" - ")
                    : "";
                  setOrderDetails((current) => ({
                    ...current,
                    tableNumber: table ? `${hall} - ${table}` : hall,
                  }));
                }}
                disabled={isSubmittingOrder}
                className={`min-w-[120px] flex-1 px-4 py-2.5 sm:py-3 rounded-lg sm:rounded-xl font-bold transition-all text-xs sm:text-sm border-2 ${
                  selectedHall === hall || (!orderDetails.tableNumber && index === 0)
      ...
      <input
        type="text"
        value={
          restaurantSettings?.halls?.length
            ? orderDetails.tableNumber.includes(" - ")
              ? orderDetails.tableNumber.split(" - ").slice(1).join(" - ")
              : ""
        ...
        onChange={(e) => {
          ...
          const halls = restaurantSettings?.halls || [];
          const currentHall = halls.find(
            (hall) => orderDetails.tableNumber === hall || orderDetails.tableNumber.startsWith(`${hall} - `)
          ) || halls[0];
          setOrderDetails((current) => ({
            ...current,
            tableNumber: currentHall
              ? tableNum ? `${currentHall} - ${tableNum}` : currentHall
              : tableNum,
          }));
        }}
        disabled={isSubmittingOrder}
        autoFocus
        placeholder="e.g., 5 or T-5 (Optional)"
      ...
      {orderDetails.tableNumber && (
        <p className="text-xs sm:text-sm text-slate-500 mt-1.5">
          Will be saved as: <span className="font-semibold text-emerald-600">{orderDetails.tableNumber}</span>
        </p>
      )}
    </div>
  )}
                      ...
                      >
                        {isSubmittingOrder ? (
                          ...
                          <>
                            ...
                            <span className="text-[10px] sm:text-xs opacity-75 hidden sm:inline">
                              (Ctrl+K)
                            </span>
                          </>
                        )}
              ...
              <div className="bg-gradient-to-r from-orange-500 via-red-500 to-pink-500 text-white p-5 sm:p-6 flex items-center justify-between flex-shrink-0">
                <div>
                  ...
                  <p className="text-orange-100 text-xs sm:text-sm mt-1">
                    {filteredPendingOrders.length} of {pendingOrders.length}{" "}
                    orders
                  </p>
                </div>
                <button
                  ...
                  <div className="space-y-3 sm:space-y-4 mb-3 sm:mb-4 flex-shrink-0">
                    ...
                    <div className="flex gap-2 sm:gap-3">
                      ...
                      <div className="relative flex-1">
                        ...
                        <input
                          ...
                          onFocus={() =>
                            editSearchQuery && setShowEditSearchDropdown(true)
                          }
                                ...
                                <div className="p-1.5 sm:p-2">
                                  {editSearchResults.map((product, index) => (
                                    <motion.button
                                      key={product._id}
                                      onClick={() =>
                                        handleEditItemSelectForQuantity(product)
                                      }
                                      onMouseEnter={() =>
                                        setSelectedEditSearchIndex(index)
                                      }
                                      className={`w-full flex items-center gap-2 sm:gap-3 p-2 sm:p-3 rounded-lg transition-all text-left ${
                        ...
                        {selectedEditItemForQuantity ? (
                          ...
                          >
                            ...
                            <div className="hidden sm:flex items-center gap-2 px-3 py-2 bg-gradient-to-r from-blue-50 to-cyan-50 border-2 border-blue-500 rounded-xl">
                              <ProductThumb item={selectedEditItemForQuantity} className="h-9 w-11" />
                              <div className="min-w-0">
                                <p className="font-bold text-slate-800 text-xs truncate max-w-[100px]">
                            ...
                            <div className="flex items-center gap-1.5 bg-white border-2 border-blue-500 rounded-xl p-1 shadow-lg">
                              <motion.button
                                ...
                                whileTap={{ scale: 0.9 }}
                                onClick={() =>
                                  setEditQuickQuantity(
                                    Math.max(0, editQuickQuantity - 1)
                                  )
                                }
                                className="p-1.5 sm:p-2 bg-red-500 text-white rounded-lg hover:bg-red-600 transition-all"
                              >
                              ...
                              <motion.button
                                ...
                                whileTap={{ scale: 0.9 }}
                                onClick={() =>
                                  setEditQuickQuantity(editQuickQuantity + 1)
                                }
                    ...
                    <AnimatePresence>
                      {selectedEditItemForQuantity && (
                        ...
                        >
                          <ProductThumb item={selectedEditItemForQuantity} className="h-10 w-12" />
                          <div className="flex-1 min-w-0">
                            <p className="font-bold text-slate-800 text-sm truncate">
                              {selectedEditItemForQuantity.name}
                            </p>
                    ...
                    {isLoadingMenuItems ? (
                      ...
                      <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 gap-2 sm:gap-3 pb-3">
                        {getEditFilteredProducts().map((item, index) => (
                          ...
                          >
                            <ProductThumb item={item} className="mx-auto h-14 w-16" />
                ...
                <div className="w-full lg:w-96 xl:w-[420px] bg-gradient-to-br from-blue-50 to-cyan-50 flex flex-col overflow-hidden">
                  ...
                  <div className="p-3 sm:p-4 lg:p-6 border-b-2 border-blue-200 flex-shrink-0">
                    ...
                    <p className="text-xs sm:text-sm text-slate-600">
                      {editCart.reduce((sum, item) => sum + item.quantity, 0)}{" "}
                      total items
                    </p>
                  </div>
                          ...
                          editCart.map((item, index) => (
                            ...
                            >
                              <div className="flex items-start gap-2 sm:gap-3">
                                ...
                                <ProductThumb item={item} className="h-10 w-12" />
                  ...
                  {editCart.length > 0 && (
                    <div className="border-t-2 border-blue-200 p-3 sm:p-4 lg:p-6 space-y-2 sm:space-y-3 bg-white/60 flex-shrink-0">
                      ...
                      <div className="space-y-2">
                        ...
                        <button
                          onClick={() =>
                            !isSavingEdit && setIsEditModalOpen(false)
                          }
                          disabled={isSavingEdit}
              ...
              <div className="bg-gradient-to-r from-emerald-500 to-teal-500 text-white p-5 flex items-center gap-3 flex-shrink-0">
                ...
                <div className="flex-1">
                  ...
                  <p className="text-emerald-100 text-sm">
                    {completionConfirmation.orderNumber}
                  </p>
                </div>
                <button
                  onClick={() => setCompletionConfirmation(null)}
                ...
                <div className="bg-gradient-to-br from-emerald-50 to-teal-50 rounded-xl p-4 border-2 border-emerald-200">
                  ...
                  <div className="space-y-2 mb-3">
                    <div className="flex justify-between text-sm">
                      ...
                      <span className="font-semibold">
                        {completionConfirmation.orderType}
                      </span>
                    </div>
                    {completionConfirmation.customerName !== "Guest" && (
                      <div className="flex justify-between text-sm">
                        ...
                        <span className="font-semibold">
                          {completionConfirmation.customerName}
                        </span>
                      </div>
                    )}
                  ...
                  <div className="bg-white/60 rounded-lg p-3 max-h-32 overflow-y-auto">
                    {completionConfirmation.items.map((item, idx) => (
                      <div
                        key={idx}
                        className="flex justify-between text-sm py-1.5 border-b border-emerald-100 last:border-0"
                      >
                        <span className="font-medium text-slate-700">
                          {item.name} x{item.quantity}
                        </span>
                ...
                <div className="bg-gradient-to-br from-slate-50 to-white rounded-xl p-4 border-2 border-slate-200">
                  ...
                  <div className="space-y-2 text-sm">
                    ...
                    {completionDetails.taxPercentage > 0 && (
                      <div className="flex justify-between text-slate-600">
                        <span>
                          Service Charges ({completionDetails.taxPercentage}%):
                        </span>
                        <span className="font-bold">
                          ₨
                          {(
                    ...
                    {completionDetails.deliveryCharge > 0 && (
                      <div className="flex justify-between text-orange-600">
                        ...
                        <span className="font-bold">
                          ₨{completionDetails.deliveryCharge.toFixed(2)}
                        </span>
                      </div>
                    )}
                    {(completionDetails.discountPercentage > 0 ||
                      completionDetails.discountAmount > 0) && (
                      <div className="flex justify-between text-emerald-600">
                        <span>
                          Discount
                          {completionDetails.discountPercentage > 0 &&
                            ` (${completionDetails.discountPercentage.toFixed(
                              1
                            )}%)`}
                          :
                        </span>
                        <span className="font-bold">
                          ...
                          {(completionDetails.discountPercentage > 0
                            ? (completionConfirmation.items.reduce(
                                ...
                                completionDetails.discountPercentage) /
                              100
                    ...
                    <div className="flex justify-between text-lg font-bold text-slate-800 pt-2">
                      ...
                      <span className="text-2xl text-emerald-600">
                        ...
                        {(() => {
                          ...
                          const tax =
                            subtotal * (completionDetails.taxPercentage / 100);
                          const discount =
                            completionDetails.discountPercentage > 0
                              ? (subtotal *
                                  completionDetails.discountPercentage) /
                          ...
                          return (
                            ...
                            tax -
                            discount +
                            completionDetails.deliveryCharge
                          ).toFixed(2);
                        })()}
                      </span>
                    </div>
