...
import { initializeGroceryStore } from '@/lib/actions/groceryStore';
  ...
  const [vendorFormData, setVendorFormData] = useState({
    ...
    const queueRenderedGroceryPrint = () => {
      requestAnimationFrame(() => requestAnimationFrame(async () => {
        const documents = Array.from(document.querySelectorAll('.print-content'));
        const content = documents.at(-1);
        if (!content) {
          showNotification('Printable receipt is not ready', 'error');
          return;
        }
        ...
        const html = `<!doctype html><html><head><meta charset="utf-8"><style>${css}
          body{margin:0;background:#fff}.print-content{display:block!important}
          .receipt-container{margin:0 auto}.restrozapp-print-footer{text-align:center;font:10px Arial;margin-top:8px}
          </style></head><body>${content.outerHTML}<footer class="restrozapp-print-footer">Powered by RestroZapp<br>restrozapp.vercel.app</footer></body></html>`;
        const result = await window.restrozapp.print.enqueueHtml({ title: 'Grocery receipt', html });
        if (!result.ok) showNotification(result.error || 'Unable to queue receipt', 'error');
      }));
    };
  ...
  const [selectedGroceryForReturnHistory, setSelectedGroceryForReturnHistory] = useState(null);
    ...
    useEffect(() => {
      initializeGroceryStore().finally(() => {
        loadInitialStats();
        loadAnalyticsData();
      });
    }, []);
    ...
    <div className="hidden print:block print-content">
      <div className="receipt-container">
        ...
        <div className="text-center mb-2 border-b-2 border-black pb-2">
          {grocerySettings?.restaurantLogo && <img src={grocerySettings.restaurantLogo} alt="" className="mx-auto mb-2 h-16 w-20 object-contain" />}
