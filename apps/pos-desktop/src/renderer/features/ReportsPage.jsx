...
const handleExportPDF = async () => {
  if (!reportsData || exportingPdf) return;
  setExportingPdf(true);
  setExportNotice(null);
  try {
    const result = await window.restrozapp.dataExport.reportPdf({
      title: `${reportType} Business Report`,
      startDate: dateFrom || undefined,
      endDate: dateTo || undefined,
    });
    setExportNotice(
      result.ok
        ? { type: 'success', message: `PDF saved to ${result.data.path}` }
        : { type: 'error', message: result.error }
    );
  } catch (exportError) {
    setExportNotice({
      type: 'error',
      message: exportError instanceof Error ? exportError.message : 'PDF export failed.'
    });
  } finally {
      ...
      <div className="max-w-[1920px] mx-auto">
        ...
        >
          <div className="flex flex-col lg:flex-row items-start lg:items-center justify-between gap-4">
            ...
            <button
              ...
              className="w-full lg:w-auto flex items-center justify-center gap-2 px-6 py-3 bg-gradient-to-r from-emerald-500 to-teal-600 text-white rounded-xl hover:from-emerald-600 hover:to-teal-700 transition-all font-semibold shadow-lg hover:shadow-xl disabled:cursor-wait disabled:opacity-60"
            >
              {exportingPdf ? (
        ...
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-3 sm:gap-4 mb-6">
          {[
            ...
            { icon: CheckCircle2, label: 'Completed', value: summary.completedOrders || 0, sub: `${summary.cancelledOrders || 0} cancelled`, gradient: 'from-violet-500 to-purple-600', border: 'border-violet-500' },
