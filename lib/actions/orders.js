...
export async function updateOrderItems(orderId, updatedItems) {
  try {
    ...
    // Validate menu items and get cost prices
    ...
    // Add cost prices and category info
...
// Add this new function for paginated orders
export async function getOrdersPaginated({ 
  ...
  try {
    ...
    if (startDate || endDate) {
      query.orderDate = {};
      if (startDate) {
        // Include full start date (00:00:00)
        const start = new Date(startDate);
        start.setHours(0, 0, 0, 0);
        query.orderDate.$gte = start;
      }
      if (endDate) {
        // Include full end date (23:59:59)
...
// Update existing getOrders to support time range
  ...
  try {
    ...
    if (filters.startDate || filters.endDate) {
      ...
      if (filters.startDate) {
        ...
        // If startTime is provided, use it, otherwise default to 00:00:00
      ...
      if (filters.endDate) {
        ...
        // If endTime is provided, use it, otherwise default to 23:59:59
        if (filters.endTime) {
...
export async function completeOrder(orderId, updatedDetails = null) {
  try {
    ...
    // ✅ FIXED: Apply updated details if provided
    if (updatedDetails) {
      ...
      if (updatedDetails.discountAmount !== undefined) order.discountAmount = updatedDetails.discountAmount;
      ...
      // Recalculate profit with new total
