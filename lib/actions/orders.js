...
export async function createOrder(orderData) {
  try {
    ...
    // ✅ Populate BOTH categoryId AND kitchenId
    ...
    // ✅ Map items with kitchen data included
    const itemsWithCost = orderData.items.map(item => {
      ...
      return {
        ...
        kitchenId: menuItem.kitchenId?._id || null,
      };
    });
    ...
    const order = new Order({
      orderNumber,
      tempOrderNumber: orderData.tempOrderNumber, // ✅ SAVE TEMP ORDER NUMBER
      items: itemsWithCost,
...
// ✅ CORRECT - Uses saved kitchenId from order
export async function reprintKOT(orderId) {
  try {
    await connectDB();
    
    // ✅ Populate the kitchenId that's SAVED in the order items
    const order = await Order.findById(orderId)
      .populate({
        path: 'items.kitchenId',
