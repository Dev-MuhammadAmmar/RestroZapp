...
export async function getKitchens() {
  try {
    ...
    const kitchens = await Promise.race([
      Kitchen.find()
        .populate({
          path: 'menuItems',
          populate: {
        ...
        .lean()
        .exec(), // Add .exec() for proper promise
    ...
    const serializedKitchens = kitchens.map(kitchen => ({
      ...kitchen,
    ...
    return { 
      success: true, 
      data: serializedKitchens 
    };
  } catch (error) {
    console.error('❌ Error fetching kitchens:', error);
    return { 
      success: false,
      error: error.message || 'Failed to fetch kitchens',
