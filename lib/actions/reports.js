...
export async function getReportsData(filters = {}) {
  try {
    ...
    const summary = {
      ...
      totalItems: 0,
      totalDiscount: 0, // ✅ ADDED: Track total discounts
      totalTax: 0, // ✅ ADDED: Track total tax
      cashPayments: 0,
      cardPayments: 0,
      onlinePayments: 0,
    ...
    orders.forEach(order => {
      ...
      if (!dailyData[dateStr]) {
        dailyData[dateStr] = {
          ...
          orders: 0,
          cost: 0,
          discount: 0, // ✅ ADDED: Track daily discounts
        };
      }
    ...
    orders.forEach(order => {
      order.items.forEach(item => {
        ...
        // ✅ FIXED: Use costPrice instead of cost
    ...
    const transactions = orders.map(order => ({
      ...
      discount: order.discount || 0, // ✅ ADDED
