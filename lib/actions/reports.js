...
export async function getReportsData(filters = {}) {
  try {
    ...
    // ✅ CHANGED: Add status filter to only get completed/ready orders
    const query = {
    ...
    // ✅ CHANGED: Updated calculations
    const completedOrders = orders.length; // All fetched orders are completed/ready
    
    // Get cancelled orders count separately for display purposes
    const cancelledOrdersQuery = { status: 'cancelled' };
    if (query.orderDate) {
      cancelledOrdersQuery.orderDate = query.orderDate;
    }
    const cancelledOrders = await Order.countDocuments(cancelledOrdersQuery);
    ...
    return {
      ...
      data: {
        ...
        chartData,
        transactions,
        orders,        // ✅ Make sure this is included
        popularItems,
        topItems,
        insights,
      },
...
export async function getOrderTypeBreakdown(filters = {}) {
  try {
    ...
    // ✅ CORRECT: Already filtering by completed/ready status
