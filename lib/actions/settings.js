...
export async function togglePrintCustomerTicket(enable) {
  try {
    await connectDB()
    const settings = await Setting.getSettings()
    settings.printCustomerTicket = enable
    await settings.save()
    
    return {
      success: true,
      message: `Print customer ticket ${enable ? 'enabled' : 'disabled'} successfully`,
      data: { printCustomerTicket: settings.printCustomerTicket },
    }
  } catch (error) {
    console.error('Error toggling print customer ticket:', error)
    return {
      success: false,
      error: 'Failed to toggle print customer ticket',
    }
  }
}
export async function toggleSplitKOTByKitchen(enable) {
  try {
    await connectDB()
    const settings = await Setting.getSettings()
    settings.splitKOTByKitchen = enable
    await settings.save()
    
    return {
      success: true,
      message: `Split KOT by kitchen ${enable ? 'enabled' : 'disabled'} successfully`,
      data: { splitKOTByKitchen: settings.splitKOTByKitchen },
    }
  } catch (error) {
    console.error('Error toggling split KOT:', error)
    return {
      success: false,
      error: 'Failed to toggle split KOT',
    }
  }
}