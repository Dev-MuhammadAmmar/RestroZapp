...
export async function makeVendorPayment(vendorName, paymentAmount, paymentMethod = 'CASH', note = '', paidBy = '') {
  try {
    ...
    for (const order of unpaidOrders) {
      ...
      // CRITICAL FIX: Ensure remainingAmount is exactly 0 when fully paid
    ...
    // CRITICAL FIX: Add delay to ensure all saves are complete before syncing
    await new Promise(resolve => setTimeout(resolve, 100));
    
    // Sync vendor stats AFTER all saves are complete
...
export async function syncVendorStats(vendorName) {
  try {
    ...
    // CRITICAL: Use fresh query to get latest data
    const purchases = await GroceryPurchase.find({ 
      vendorName: vendorName,
      isArchived: false 
    }).lean();
    ...
    const stats = {
      ...
      totalPurchaseValue: purchases.reduce((sum, p) => sum + p.totalAmount, 0),
      totalPaid: purchases.reduce((sum, p) => sum + (p.paidAmount || 0), 0),
      totalPending: purchases.reduce((sum, p) => sum + (p.remainingAmount || 0), 0),
      lastOrderDate: purchases.length > 0 ? 
        purchases.sort((a, b) => new Date(b.createdAt) - new Date(a.createdAt))[0].createdAt : 
        null
    };
    
    // CRITICAL FIX: Round totalPending to avoid floating point issues
    stats.totalPending = Math.max(0, Math.round(stats.totalPending * 100) / 100);
    ...
    console.log(`✓ Vendor stats synced: ${vendorName} - Pending: ₨${stats.totalPending}`);
